...
<body>
	...
	try {
		...
		String newhousenumber = request.getParameter("housenumber");
		...
		String str = "SELECT * FROM Address WHERE housenumber = ? AND street = ? AND state = ? AND zipcode = ? AND country = ? ";
		PreparedStatement ps1 = con.prepareStatement(str);
		ps1.setString(1, newhousenumber);
		ps1.setString(2, newstreet);
		ps1.setString(3, newstate);
		ps1.setString(4, newzipcode);
		ps1.setString(5, newcountry);
		
		ResultSet result = ps1.executeQuery();
		if(result.next() == true){
			out.print("The address is already in use!");
		}
		else{
			
		str = "INSERT INTO Address(housenumber, street, state, zipcode, country)"+ "VALUES (?,?,?,?,?)";
		PreparedStatement ps2 = con.prepareStatement(str);
		// ps2.setInt(1, newuserid);
		ps2.setString(1, newhousenumber);
		ps2.setString(2, newstreet);
		ps2.setString(3, newstate);
		ps2.setString(4, newzipcode);
		ps2.setString(5, newcountry);
		ps2.executeUpdate();
