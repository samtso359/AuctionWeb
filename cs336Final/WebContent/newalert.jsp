...
<body>
	...
	try {
		...
		String newalertname = request.getParameter("name");
		String newconditions = request.getParameter("conditions");
		...
		if(result.next() == true){
			out.print("The username already exists!");
		}
		else{
			
			*/
		String str = "INSERT INTO Alert(name, conditions, price, itemname,username)"+ "VALUES (?,?,?,?,?)";
		PreparedStatement ps2 = con.prepareStatement(str);
		// ps2.setInt(1, newuserid);
		ps2.setString(1, newalertname);
		ps2.setString(2, newconditions);
		ps2.setInt(3, newprice);
		ps2.setString(4, newitemname);
		ps2.setString(5, newusername);
		
		

		ps2.executeUpdate();
			out.print("Alert has been created");
